 ...
 *
 */
@ManagedBean(name = "graficaHorasBean")
public class GraficaHorasBean extends GraficInterfaz implements Serializable {
	private static final long serialVersionUID = -2296350223299578082L;
	private static final Logger log = Logger.getLogger(GraficaHorasBean.class);
	...
	public void init() {
		setIntervalo("100");
		if (seleccionBean.getAuxiliarGrafica() == 6 || seleccionBean.getAuxiliarGrafica() == 0) {
			buscar();
		}
	}
	...
	public void obtenerValores() {
		...
		datosGraficas.setHorasMaqEstimadas(datos.get(0));
		datosGraficas.setHorasMaqReales(datos.get(1));
	}
	...
	public LineChartModel initModels() {
		...
		try {
			if (dialogoGrafica == false) {
				...
				datosGraficas = graficasFacade.obtenerTotalBackLogsHorasMaquinasHorasReales(fecha1, fecha2, serie,
						modelo, almacen);
			} else {
				obtenerValores();
			}
		...
		horasEstimadas.setLabel("HORAS MÁQUINA ESTIMADAS");
		crearColumnasDouble(horasEstimadas, fecha1, fecha2, datosGraficas.getHorasMaqEstimadas());

		horasReales.setLabel("HORAS MÁQUINA REALES");
		crearColumnasDouble(horasReales, fecha1, fecha2, datosGraficas.getHorasMaqReales());
	...
	public void crearValoresGrafica() {
		String valores = "[";
		for (Double val : datosGraficas.getHorasMaqEstimadas()) {
			if (valores.equals("[")) {
				valores = valores + "" + val;
			} else {
				valores = valores + "," + val;
		...
		valores = valores + "],[";
		for (int i = 0; i < datosGraficas.getHorasMaqReales().size(); i++) {
			if (i == 0) {
				valores = valores + "" + datosGraficas.getHorasMaqReales().get(i);
			} else {
				valores = valores + "," + datosGraficas.getHorasMaqReales().get(i);
			}
		}
	...
	public void createModels() {
		...
		lineChartModel.setTitle("ADMINISTRACIÓN DE BACKLOGS - TIEMPO ESTIMADO DE REPARACIÓN ");
	...
	public int getMaximo() {
		...
		for (double valor : datosGraficas.getHorasMaqEstimadas()) {
			max = Math.max(max, valor);
		}

		for (double valor : datosGraficas.getHorasMaqReales()) {
			max = Math.max(max, valor);
		}
